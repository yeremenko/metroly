define([
	...
	var MapView = Backbone.View.extend({
		...
		initialize: function() {
			...
			this.model.on("change:route", _.bind(this.selectedRouteChanged, this));	
			this.model.on("change:directionId", _.bind(this.selectedRouteChanged, this));
		},

		selectedRouteChanged: function() {
			var route = this.model.get('route');
			var direction = this.model.get('directionId');
			this.showBuses(route, direction);
		},
		...
		showBuses: function(route, directionId) {
			this.resetLayers();
			var routeId = route.shortname.toUpperCase();
			metroly.getBuses(routeId, directionId, _.bind(this.markBuses, this));
			// Populate the route layers.
		...
		showRoute: function(route) {
			...
			for (var i = 0; i < route.routeDirections.length; i += 1) {
				...
				var latlngs = [];
				_.each(routeDirection.polylines, function(polyline){
